...
function localizedBackendMessage(message: string | null, language: Language, provider: ProviderSnapshot["provider"]): string | null {
  ...
  if (normalized.includes("sign in") || normalized.includes("login")) return provider === "claude" ? "Claude 登录已失效，请重新连接。" : "Codex 登录已失效，请重新登录。";
    ...
    >
      ...
      {available && displayPercent !== null ? (
          ...
          <footer className="card-footer">
            <div className="weekly-metric">
              ...
              {snapshot.provider === "claude" ? (
                <div className="reset-credit-row"><span>{snapshot.subscriptionExpiresAt ? t.subscriptionUntil(formatResetDate(snapshot.subscriptionExpiresAt, language)) : t.claudeQuotaSource}</span></div>
              ) : (
              ...
              {snapshot.provider === "codex" && showCreditTip ? (
        ...
        <section className="error-state" aria-live="polite">
          ...
          <strong>{snapshot.status === "signed_out" ? (snapshot.provider === "claude" ? t.claudeSignInRequired : t.signedInRequired) : staleExpired ? t.staleExpired : t.temporarilyUnavailable}</strong>
          <p>{message ?? t.errorUnavailable}</p>
          {snapshot.status === "signed_out" && snapshot.provider === "claude" ? (
            <button type="button" className="error-refresh-button" onMouseDown={(event) => event.stopPropagation()} onClick={onConnectClaude} disabled={!onConnectClaude} aria-label={t.connectClaude}>
              <SignIn />
              <span>{t.connectClaude}</span>
            </button>
          ) : null}
          {snapshot.status === "stale" ? (
...
export const QuotaOrb = memo(function QuotaOrb({ snapshot, onDrag, onHover, language = "zh-CN" }: Pick<Props, "snapshot" | "onDrag" | "onHover"> & { language?: Language }) {
  ...
  return (
    <main
      ...
      onMouseLeave={() => onHover(false)}
      onMouseDown={(event) => { if (event.button === 0) void onDrag(); }}
      aria-label={available ? (displayingWeeklyAsPrimary ? t.weeklyAvailableLabel(displayPercent!) : t.availableLabel(displayPercent!)) : localizedBackendMessage(snapshot.message, activeLanguage, snapshot.provider) ?? t.unavailableStatus}
    >
      <div className="aurora" aria-hidden="true" />
