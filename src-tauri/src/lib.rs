...
async fn fetch_snapshots_uncached(
    ...
    let (codex, claude) = tokio::join!(
        ...
        claude::fetch_snapshot(&state.client, app),
    );
    let values = vec![codex, claude];
...
async fn get_snapshots(
    ...
    let _guard = match state.fetch_lock.try_lock() {
        ...
        Err(_) => {
            ...
            return Ok(vec![
                ProviderSnapshot::failure("unavailable", "Quota refresh is already running."),
...
async fn refresh_snapshots(
    ...
    Ok(fetch_snapshots_uncached(&app, &state).await)
}
...
fn setup_tray(app: &tauri::App) -> tauri::Result<()> {
    ...
    let connect_claude =
        MenuItem::with_id(app, "connect-claude", "Connect Claude", true, None::<&str>)?;
    let disconnect_claude = MenuItem::with_id(
        app,
        "disconnect-claude",
        "Disconnect Claude",
        true,
        ...
        .on_menu_event(move |app, event| match event.id.as_ref() {
            ...
            "connect-claude" => {
                ...
                tauri::async_runtime::spawn(async move {
                    let _ = claude_auth::connect_claude(app).await;
                });
            }
            "disconnect-claude" => {
                let app = app.clone();
                tauri::async_runtime::spawn(async move {
                    if claude_auth::disconnect_claude(app.clone()).await.is_ok() {
                        let _ = app.emit_to("widget", "refresh-requested", ());
                    }
                });
            }
                ...
                if let Some(state) = app.try_state::<AppState>() {
                    if let Ok(mut prefs) = state.preferences.lock() {
                        ...
                        *prefs = normalized.clone();
                        ...
                        let _ = connect_claude_menu.set_text(if english {
                            "Connect Claude"
                        } else {
                            "连接 Claude"
                        });
                        let _ = disconnect_claude_menu.set_text(if english {
                            "Disconnect Claude"
                        } else {
                            "断开 Claude"
                        });
...
pub fn run() {
    let app = tauri::Builder::default()
        ...
        .expect("failed to build Quota Assistant");
